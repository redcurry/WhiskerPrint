...
{
    public class Chamfer : IMatcherAlgorithm
    {
        ...
        private static int OptRadius = 8;
        private static int OptRadius2 = 16;
        ...
        private double GetDirectedScore(Image image1, Image image2)
        {
            ...
            unsafe
            {
                ...
                if (_distanceMethod == DistanceMethods.Median)
                {
                ...
                for (j = image1Top; j <= image1Bottom; j++)
                {
                    for (i = image1Left; i <= image1Right; i++)
                    {
                        ...
                        if (_distanceMethod == DistanceMethods.Mean)
                        {
                            if (image1Ptr[0] == 0)
                            {
                                double d = getNearestDistanceToColor(0, image2PtrOrigin,
                                ...
                                /*
                            else if (image1Ptr[0] == 255)
                            {
                                distance += getNearestDistanceToColor(255, image2PtrOrigin,
                                    width2, height2, i, j, bpp, stride2);
                            }
                                 */
            ...
            {
                if (distance == double.PositiveInfinity || distance == double.MaxValue)
                {
                    return 0.0;
                }
        ...
        private unsafe double getNearestDistanceToColor(int color, byte* image2PtrOrigin, int width, int height, int i1, int j1, int bpp, int stride2)
        {
            double minDistance = double.MaxValue;
            ...
            // Find the minimum distance within a small neighborhood
            minDistance = getNeighborhoodMinDistanceToColor(color, i1, j1, OptRadius,
                image2PtrOrigin, width, height, bpp, stride2);
            ...
            // Find the minimum distance within a bigger neighborhood
            minDistance = getNeighborhoodMinDistanceToColor(color, i1, j1, OptRadius2,
                image2PtrOrigin, width, height, bpp, stride2);
            ...
            // Give up optimization, and search everything
            minDistance = getCompleteMinDistanceToColor(color, i1, j1,
                image2PtrOrigin, width, height, bpp, stride2);
        ...
        private unsafe double getCompleteMinDistanceToColor(int color, int i1, int j1,
            byte* image2PtrOrigin, int width, int height, int bpp, int stride2)
        {
            // Start by assuming the minumum distance is very large
            double minDistance = double.MaxValue;
            ...
            for (j2 = image2Top; j2 <= image2Bottom; j2++)
            {
                for (i2 = image2Left; i2 < image2Right; i2++)
                {
                    ...
                    // Examine only pixels of the specified color
                    if (image2Ptr[0] == color)
                    {
                        // Get the distance between pixels
                        distance = getPixelDistance(i1, j1, i2, j2);
        ...
        private unsafe double getNeighborhoodMinDistanceToColor(int color, int i1, int j1,
            int radius, byte* image2PtrOrigin, int width, int height, int bpp, int stride)
        {
            // Calculate where neighborhood starts and ends
            int i2Start = i1 - radius;
        ...
        {
            //return (i2 - i1) * (i2 - i1) + (j2 - j1) * (j2 - j1);
        ...
        private DistanceMethods _distanceMethod = DistanceMethods.Median;
    }
}
